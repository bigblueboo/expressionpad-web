...
public func applyPreset(_ name: String, to store: Store) {
    ...
    store.patch { s in
        s.set(\.synth.preset, name)
        s.set(\.synth.gen1.morph, p.gen1.morph)
        s.set(\.synth.gen1.semi, p.gen1.semi)
        s.set(\.synth.gen1.tune, p.gen1.tune)
        s.set(\.synth.gen1.level, p.gen1.level)
        s.set(\.synth.gen2.morph, p.gen2.morph)
        s.set(\.synth.gen2.semi, p.gen2.semi)
        s.set(\.synth.gen2.tune, p.gen2.tune)
        s.set(\.synth.gen2.level, p.gen2.level)
        s.set(\.synth.bright, p.bright)
        s.set(\.synth.env.a, p.env.a)
        s.set(\.synth.env.d, p.env.d)
        s.set(\.synth.env.s, p.env.s)
        s.set(\.synth.env.r, p.env.r)
        s.set(\.synth.filter.cutoff, p.filter.cutoff)
        s.set(\.synth.filter.res, p.filter.res)
        s.set(\.synth.filter.env, p.filter.env)
        s.set(\.synth.lfo.rate, p.lfo.rate)
        s.set(\.synth.lfo.depth, p.lfo.depth)
        s.set(\.synth.lfo.target, p.lfo.target)
    }
}
